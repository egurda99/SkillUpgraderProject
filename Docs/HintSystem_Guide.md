...
> Ядро модуля вынесено как переиспользуемый пакет-модуль `Hint_System` в `U:\Unity\MyCodeBase\Runtime\Hint_System` — вместе с `Tutorial_System` он документирован там же в `README.md` как единый «Onboarding»-контур. Project-специфичный `UpgradeHintInspector` в пакет не переносился как часть ядра — он лежит там же отдельным семплом (`Samples~/Hint_System`).
