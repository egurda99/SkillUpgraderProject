...
>
> Ядро модуля (без project-специфичного `Content/`) вынесено как переиспользуемый пакет-модуль `Tutorial_System` в `U:\Unity\MyCodeBase\Runtime\Tutorial_System` — вместе с `Hint_System` он документирован там же в `README.md` как единый «Onboarding»-контур. Конкретные шаги (`Content/`) в пакет не переносились и остаются в этом проекте как есть; в пакете вместо них — семпл `Samples~/Tutorial_System` (тот же 6-шаговый пример, взятый из этого проекта).
