...
namespace Audio
{
    public sealed class AudioManager : IDisposable
    {
        ...
        public float GetRawMixerVolume(AudioOutput output)
        {
            string channelName = output switch
            {
                AudioOutput.None => AudioManagerStaticData.SOUNDS_MAIN_CHANNEL_NAME,
            ...
            if (AudioMixer.GetFloat(channelName, out float value))
                return value;

            Debug.LogWarning($"AudioMixer channel not found: {channelName}");
            return 0f;
        }
